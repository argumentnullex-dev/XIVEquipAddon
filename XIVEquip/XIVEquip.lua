...
f:SetScript("OnEvent", function(_, event, arg1)
  if event == "ADDON_LOADED" and arg1 == addonName then
    ...
    s.Weapons.Mode = s.Weapons.Mode or
        "AUTO"                                -- AUTO | TWOHAND | DUAL_1H | DUAL_2H | MH_SHIELD | MH_OFFHAND | SOLO_1H
  ...
  elseif event == "PLAYER_LOGIN" then
    ...
    if status == "default:pawn" then
      msgLogin(L.Loaded_Default_Pawn)
    elseif status == "default:ilvl" then
      msgLogin(L.Loaded_Default_Ilvl)
    elseif status == "saved:ok" then
      msgLogin(string.format(L.Loaded_Using_Name, tostring(active)))
    elseif status == "saved:unknown" then
      msgLogin(L.Warn_Unknown)
    elseif status == "saved:unavailable" then
      msgLogin(L.Warn_Unavailable)
    else
      msgLogin(string.format(L.Loaded_Using_Name, tostring(active or "ilvl")))
    end
  end
end)
